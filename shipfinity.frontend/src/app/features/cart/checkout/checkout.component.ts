...
export class CheckoutComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(){
    ...
    this.shoppingCartSub = this.shoppingCartService.getCart().subscribe(data => {
      this.cartItems = data;
    });
  }
  ...
  paymentFormSubmitted(){
    const order: IOrder = {
      ...
      orderDetails: this.cartItems.map(item => { return { productId: item.product.id, quantity: item.quantity } })
    }
